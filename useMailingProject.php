...
<head lang="en">
    ...
    <title>SMS - Use mailing projects</title>
    ...
    <div class="content">
        ...
        if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (isset($_POST["loadProject"])) {
                    if (isset($_POST["selectProject"])) {
                        $selectedProject = $_POST["selectProject"];

                        $ret = selectProjectById($selectedProject);
                        if ($ret != false) {
                        ...
                        $ccEmail = prepare_data(getEmailAddressesFromProject($selectedProject));
                    }
            ...
            <form class="pure-form pure-form-stacked" name="mailForm" method="post" enctype="multipart/form-data"
                  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                <fieldset>
                    <p>Choose mailing project:
                            ...
                            if ($ret != false) {
                                while ($row = $ret->fetchArray(SQLITE3_ASSOC)) {
                                    ...
                                    if ($rowId == $selectedProject) {
                                        echo "<option value=\"$rowId\" selected=\"selected\">$rowName</option>";
                                    } else {
                        ...
                        </br><input class="pure-button" type="submit" name="loadProject" value="Load project"/>
                    </p>
                    ...
                    <p>
                        <input class="button-success pure-button" type="submit" name="sendMail" value="Send"
                               onclick="return checkMailFields()"/>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (isset($_POST["sendMail"])) {
                ...
                if (sendMailPhpMailer($_POST["email"], $_POST["ccEmail"], $_POST["bccEmail"], $_POST["subject"], $_POST["message"], $_FILES["item_file"], $_POST["selectProject"])) {
